    ...
    return (
        <HoverCard openDelay={300}>
            ...
            <HoverCardContent 
                side="right" 
                align="start" 
                className="w-[400px] p-3 bg-white border shadow-lg"
            >
                <div className="space-y-3">
                    <div className="flex items-center justify-between">
                        ...
                        <div className="text-xs text-gray-500 px-2 py-1 bg-gray-50 rounded-full flex-shrink-0">
                            {dashboard.widgets.length} widgets
                        </div>
                    </div>
                    <div 
                        className="grid gap-1.5 bg-gray-50/50 p-2 rounded-lg" 
                        style={{
                    ...
                    >
                        {dashboard.widgets.map((widget: any, index: number) => (
                            <div
                                ...
                                className="bg-white border shadow-sm rounded-md p-1.5 flex flex-col items-center justify-center gap-1.5 hover:border-blue-200 hover:shadow-md transition-all group"
                                title={widget.config.name}
                            >
                                {getWidgetIcon(widget.config.type)}
                                {widget.position.sizey > 1 && (
                                    <div className="w-full px-1">
                                        <div className="text-[10px] text-gray-600 text-center leading-tight truncate group-hover:text-blue-600">
                                            {widget.config.name}
                                        </div>
                                    </div>
                                )}
