...
const ComputeRequirements: React.FC<ComputeRequirementsProps> = ({ 
  ...
  const ResourceSection = ({ 
    ...
    <div className={`bg-white dark:bg-gray-900 border border-${color}-100 dark:border-${color}-800 rounded-lg p-3 hover:bg-${color}-50/50 dark:hover:bg-${color}-900/20`}>
      {/* Mobile Layout */}
      <div className="sm:hidden space-y-3">
        <div className={`flex items-center gap-2`}>
          <div className={`w-6 h-6 rounded-full bg-${color}-50 dark:bg-${color}-900/20 flex items-center justify-center flex-shrink-0`}>
            <Icon className={`w-3 h-3 text-${color}-700 dark:text-${color}-400`} />
          </div>
          <span className={`text-sm font-medium text-${color}-900 dark:text-${color}-100 truncate`}>{title}</span>
        </div>
        <div className="grid grid-cols-3 gap-2">
          ...
          <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-2">
            ...
            <div className="text-sm font-medium text-gray-900 dark:text-gray-100">{totals.cpu}</div>
          </div>
          {/* Memory */}
          <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-2">
            <div className="flex items-center gap-1.5 mb-1">
            ...
            <div className="text-sm font-medium text-gray-900 dark:text-gray-100">{totals.memory} GB</div>
          </div>
          {/* Disk */}
          <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-2">
            <div className="flex items-center gap-1.5 mb-1">
            ...
            <div className="text-sm font-medium text-gray-900 dark:text-gray-100">{totals.disk} GB</div>
          </div>
        </div>
      </div>
      ...
      <div className="hidden sm:grid grid-cols-[150px_1fr_1fr_1fr] items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <div className="flex items-center gap-2">
            <Cpu className="w-3 h-3 text-gray-500" />
            <span className="text-sm font-medium text-gray-900 dark:text-gray-100">{totals.cpu}</span>
          </div>
        </div>
        ...
        <div className="flex-1">
          ...
          <div className="flex items-center gap-2">
            <MemoryStick className="w-3 h-3 text-gray-500" />
            <span className="text-sm font-medium text-gray-900 dark:text-gray-100">{totals.memory} GB</span>
          </div>
        </div>

        {/* Disk Column */}
        <div className="flex-1">
          <div className="text-xs text-gray-500 dark:text-gray-400 mb-1">Disk</div>
          <div className="flex items-center gap-2">
            <HardDrive className="w-3 h-3 text-gray-500" />
            <span className="text-sm font-medium text-gray-900 dark:text-gray-100">{totals.disk} GB</span>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div className={`bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4 ${className}`}>
      <h3 className="font-medium text-gray-900 dark:text-gray-100 mb-3">Compute Requirements</h3>
      
      <div className="space-y-2">
        {(!hasPollingResources && !hasLogsResources && !hasNetflowResources) ? (
          <div className="flex items-center gap-2 p-3 bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-lg text-gray-500 dark:text-gray-400 text-sm">
